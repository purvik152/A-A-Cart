...
const OrderSummary = () => {
  const { currency, router, getCartCount, getCartAmount, placeOrder } = useAppContext();
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      // No need to handle success here as placeOrder will redirect
    } catch (error) {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (isDropdownOpen && !event.target.closest('.address-dropdown')) {
        setIsDropdownOpen(false);
      }
    };
    ...
    <div className="w-full md:w-96 bg-gray-500/5 p-5">
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="relative inline-block w-full text-sm border address-dropdown">
            <button
        ...
        <div>
          ...
          <div className="flex flex-col items-start gap-3">
            ...
            />
            <button className="bg-orange-600 text-white px-9 py-2 hover:bg-orange-700 transition">
              Apply
            </button>
          </div>
        </div>
